...
def run_experiment( method_option: str = "best", output_prefix: str = "", with_synpop_cart = True):
    ...
    signal = pd.DataFrame(df["signal"], columns=['signal'])
  
    X_train, X_test, y_train, y_test = train_test_split(
    df.drop(columns='signal'), signal, test_size=0.3, random_state=42)
    ...
    METHODS = METHODS_SYNTHGEN + [
        ...
        "statis_dual_energy_eigen", 
        "statis_dual_energy_weight"
    ]
...
# Évaluation des métriques
    def evaluate_metrics(method, df_synth):
        ...
        print("****")
        print(energy(df_synth, df_true_complete, scale=True))
        print(wasserstein_dist(df_synth,df_true_complete, scale=True))
        print("************")
        ...
        clf.fit(df_synth.drop(columns='signal'), df_synth['signal'])
...
# Boucle principale
    for i in range(n_synth):
        ...
        for method in METHODS_SYNTHGEN:
            ...
            df_synth['signal'] = (df_synth['signal'] > 0.5).astype(int)
    ...
    # Agrégation mixture
        for w_method in ["energy"]:
            ...
            df_mix['signal'] = (df_mix['signal'] > 0.5).astype(int)
    ...
    # Agrégation statis
        for key, params in statis_param.items():
            ...
            df_stat['signal'] = (df_stat['signal'] > 0.5).astype(int)
