...
def run_experiment( method_option: str = "best", output_prefix: str = "", with_synpop_cart = True):
    ...
    df = pd.read_csv("/home/mfbeclin/synthetic_data/synthetic_data/synthetic_proj_python/test_on_simulated_dataset/data_simulated/diabetes.csv")
    
   
    X_train, X_test, y_train, y_test = train_test_split(
    
    df.drop(columns='Outcome'), df['Outcome'], test_size=0.25, random_state=42)
    n= len(X_train)
...
# Évaluation des métriques
    def evaluate_metrics(method, df_synth):
        ...
        clf.fit(df_synth.drop(columns='Outcome'), df_synth['Outcome'])
...
# Boucle principale
    ...
    # Agrégation mixture
        for w_method in ["wasserstein", "energy"]:
            ...
            df_mix['Outcome'] = (df_mix['Outcome'] > 0.5).astype(int)
    ...
    # Agrégation statis
        for key, params in statis_param.items():
            ...
            df_stat['Outcome'] = (df_stat['Outcome'] > 0.5).astype(int)
...
# Sauvegarde
    file_name = OUT_PATH / f"diabetes_classif_n={n_synth}.pkl"
