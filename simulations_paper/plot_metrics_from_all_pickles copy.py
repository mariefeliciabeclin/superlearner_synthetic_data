...
def plot_metric_grouped_by_pickle(metric_key, data_list, methods, method_labels, output_dir):
    import matplotlib.pyplot as plt
    import numpy as np
    import os
    ...
    fig, ax = plt.subplots(figsize=(20, 4 * num_pickles + 1))
    ...
    ax.set_yticklabels([DATASET_LABS[str.split(name, sep='best')[0].replace("_", " ")] for name, _ in data_list], fontsize=30)
    #print([str.split(name, sep='_')[0]+ " "+str.split(name, sep='_')[1] for name, _ in data_list])
    ax.set_xlabel("Metric Value", fontsize=25)
    ax.set_title(metric_key.replace("_", " ").title(), fontsize=28, fontweight='bold')
    ...
    #if metric_key == "metric_energy_scaled":
    ax.legend(
        handles,
        method_labels,
    ...
    plt.tight_layout()
    fig.savefig(os.path.join(output_dir, f"grouped_by_pickle_{metric_key}_all.pdf"), dpi=300)
    plt.close()
    print(f"Saved: grouped_by_pickle_{metric_key}.pdf")
...
def plot_all_grouped(folder_path, output_dir, order_file=False):
    ...
    for metric_key in metrics:
        plot_metric_grouped_by_pickle(metric_key, data_list, METHODS, METHOD_LABELS, output_dir)
