...
def plot_bias_two_rows(lowcorr_stats, highcorr_stats, methods, var_list, output_path=None):
    ...
    for j, var in enumerate(var_list):
        for row, (stats, conf_size, row_label) in enumerate([
            ...
            ax.axvline(x=conf_size[j] / 2, color='grey', linestyle='--')
            ax.axvline(x=-conf_size[j] / 2, color='grey', linestyle='--')
...
if __name__ == "__main__":
    res_high = load_biases(OUT_PATH / "highcorr_n=100.pkl")
    res_low = load_biases(OUT_PATH / "lowcorr_n=100.pkl")
    ...
    low_stats = extract_bias_stats(res_low, VAR_LIST)
    high_stats = extract_bias_stats(res_high, VAR_LIST)
