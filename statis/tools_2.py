...
def compromis_V(DF, delta=None,compromis_method = "eigen", delta_weight = True):
    ...
    for df in DF:
        ...
        V.append((np.array(df_new).T)@D@(np.array(df_new)))
...
# Extraire la plus grande valeur propre et son vecteur propre
    indice = 0
    while not np.all(np.array(E["vect"].iloc[indice]) >= 0):
        indice = indice+1
    tau_1 = np.array(E['vect'].iloc[indice])
    print(tau_1)
...
def compromis_W(DF, delta=None, compromis_method = "eigen", delta_weight = True):
    DF_scaled=[]
    W=[]
...
# Extraire la plus grande valeur propre et son vecteur propre
    indice = 0
    while np.all(np.array(E["vect"].iloc[indice])> 0) is False:
        indice = indice+1
    tau_1 = np.array(E['vect'].iloc[indice])
    tau_1 = (1/np.sum(tau_1))*tau_1
