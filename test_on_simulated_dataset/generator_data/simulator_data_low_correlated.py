...
def generate_lowcorr_data(seed=123,n=500,
                           mean=None,
                           cov_matrix=None,
                           beta_real=None,):
    np.random.seed(seed)
...
def generate_and_save_data(n=500,
    ...
    df =  generate_lowcorr_data(n=500, mean=mean, cov_matrix=cov_matrix, beta_real=beta_real)
...
def main():
    ...
    generate_and_save_data(
        n=500,
        output_csv_path=base_path / "simulated_data_lowcorr.csv",
        output_pairplot_path=base_path / "pairplot_lowcorr.pdf",
        output_corrplot_path=base_path / "correlation_lowcorr.pdf"
    )
